...
static uint64
argraw(int n) // helps retrieve the appropriate saved user register.
{
  struct proc *p = myproc(); // my proc helps in finding the register values of the current process
  switch (n)
  {
  case 0:
...
// The functions argint, argaddr, and argfd retrieve the nth system call argument
...
// corresponding to the system call numbers
char *syscall_name[] = {
    [SYS_fork] "fork",
    [SYS_exit] "exit",
    [SYS_wait] "wait",
...
// to each system call in the above order of system calls.
int syscall_argc[] = {
    [SYS_fork] 0,
    [SYS_exit] 1,
    [SYS_wait] 1,
